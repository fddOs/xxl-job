...
public class MybatisConfigurer {
	...
	public SqlSessionFactory sqlSessionFactoryBean(DataSource dataSource) throws Exception {
		 ...
		 * Properties(); properties.setProperty("pageSizeZero",
		 * "true");//分页尺寸为0时查询所有纪录不再执行分页 properties.setProperty("reasonable",
		 * "true");//页码<=0 查询第一页，页码>=总页数查询最后一页
		 * properties.setProperty("supportMethodsArguments", "true");//支持通过
	...
	public MapperScannerConfigurer mapperScannerConfigurer() {
		...
		// "false");//insert、update是否判断字符串类型!='' 即 test="str != null"表达式内是否追加
		// and str != ''
